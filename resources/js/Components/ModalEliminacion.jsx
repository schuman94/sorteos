    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg">
                ...
                {mensaje && (
                    <p className={`mb-6 text-sm ${error ? 'text-red-600 font-medium' : 'text-gray-700'}`}>
                        {mensaje}
                    </p>
                )}

                <div className="flex justify-end gap-2">
                    <BotonGris onClick={onCancelar}>Cancelar</BotonGris>
                    <BotonRojo onClick={onConfirmar}>Confirmar</BotonRojo>
                </div>
            </div>
        </div>
    );
